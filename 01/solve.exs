  ...
  def solve_02() do
    ...
    |> reduce(
      {0, 50}, fn x, { clicks, dial } -> 
        next_dial = rem(rem(x, 100) + dial + 100, 100)
        cond do
          next_dial == 0 ->
            { clicks + 1 + div(abs(x), 100), next_dial }
          x < 0 and next_dial > dial and dial != 0 ->    
            { clicks + 1 + div(abs(x), 100), next_dial }
          x > 0 and next_dial < dial ->
            { clicks + 1 + div(abs(x), 100), next_dial }
          true ->
            { clicks + div(abs(x), 100), next_dial }
        end
      end
    )
    |> elem(0)
